...
"""
    ...
    try:
        ...
        if "candidates" not in res_data or not res_data["candidates"]:
            ...
            print(f"🚫 Gemini Blocked: {error_msg}")
...
def analyze_issue(data: IssueRequest):
    try:
        print(f"\n📌 API REQUEST: {data.issue_text}")
        ai_response = gemini_analysis(data.issue_text)
        print(f"✅ RETURNING AI RESPONSE")
        return ai_response
    except Exception as e:
        print(f"⚠️ Gemini failed, using fallback: {str(e)}")
        translated_text = translate_to_english(data.issue_text)
        print(f"🌐 Translated for fallback: {translated_text}")

        fallback_response = rule_based_fallback(translated_text)
